...
\begin{tcolorbox}[title=Note]
	...
	Because:\\
	{\emoji 👉} AWS VPC already provides DHCP\\
	{\emoji 👉} DHCP broadcast is blocked in AWS\\
\end{tcolorbox}
...
# If in a domain (skip if standalone)
# Add-DhcpServerInDC -DnsName "dhcpserver.example.internal" -IpAddress "10.0.1.10"
...
Set-DhcpServerv4OptionValue -ScopeId 10.0.1.0 -DnsDomain "example.internal"

New-NetFirewallRule -DisplayName "Allow DHCP UDP 67" -Direction Inbound -Protocol UDP -LocalPort 67 -Action Allow
New-NetFirewallRule -DisplayName "Allow DHCP UDP 68" -Direction Inbound -Protocol UDP -LocalPort 68 -Action Allow
\end{lstlisting}

\subsection{Usage}
\begin{itemize}
	\item AWS VPC DHCP is used in EC2 instance
\end{itemize}
\clearpage
\begin{figure}[htbp]
	\includegraphics[width=\textwidth]{static/dhcp_server/dhcp_server.png}
	\caption{A dhcp server instance}
\end{figure}
...
\subsection{AWS Configuration}
Instance Name: Mail Server\\
Instance Type: t3.medium or larger\\
Security Group Ports: tcp 25 (SMTP), tcp 143 (IMAP), tcp 3389 (RDP)
...
Instance Type: t3.medium\\
Security Group Ports: tcp 80, tcp 3389 (RDP)
...
\begin{itemize}
	...
	\item
	      \begin{itemize}
		      \item include Server 1 \& Server 2
	      \end{itemize}
...
\subsection{AWS Configuration}
Instance Name: Failover Cluster\\
Instance Type: t3.medium or larger\\
Storage: Shared storage using FSx for Windows, S3, iSCSI (Internet Small Computer Systems Interface), or Storage Spaces Direct (S2D)\\
...
\subsubsection{EVERY NODE}
\begin{lstlisting}[language=bash]
# ================================
# Script 1: Run on EVERY NODE
# ================================
Write-Host "Installing Failover Clustering feature..." -ForegroundColor Cyan
Install-WindowsFeature Failover-Clustering -IncludeManagementTools -Restart
\end{lstlisting}
\subsubsection{ONE NODE ONLY}
\begin{lstlisting}[language=bash]
# ================================
# Script 2: Run on ONE NODE ONLY
# ================================
...
# ---- 4. Add disks & enable CSV ----
Write-Host "Checking for available disks..." -ForegroundColor Cyan
$AvailableDisks = Get-ClusterAvailableDisk
          
if ($AvailableDisks) {
    Write-Host "Adding disks to Cluster Shared Volumes..." -ForegroundColor Cyan
    $AvailableDisks | Add-ClusterDisk
    $AvailableDisks | Add-ClusterSharedVolume
} else {
    Write-Host "No available disks found. Skipping CSV." -ForegroundColor Yellow
}
          
# ---- 5. Configure Quorum ----
Write-Host "Configuring quorum (Node Majority)..." -ForegroundColor Cyan
Set-ClusterQuorum -NodeMajority
          
# ---- DONE ----
Write-Host "====================================="
Write-Host "  Failover Cluster Creation Complete!"
...
\subsection{Usage}
\begin{itemize}
	\item there are 3 servers
	\item 1 server for domain controller
	\item 2 other servers join the domain and work as a cluster
\end{itemize}

\begin{figure}[htbp]
	\includegraphics[width=\textwidth]{static/failover_cluster/2_nodes.png}
	\caption{2 nodes are up in a cluster}
\end{figure}
\clearpage
\begin{figure}[htbp]
	\includegraphics[width=\textwidth]{static/failover_cluster/failover_cluster_for_file_server.png}
	\caption{Failover cluster for file server using Storage Spaces Direct (S2D)}
\end{figure}
\clearpage
\begin{figure}[htbp]
	\includegraphics[width=\textwidth]{static/failover_cluster/Failover Cluster.drawio.pdf}
	\caption{A cluster having two nodes}
\end{figure}
